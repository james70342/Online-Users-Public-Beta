...
class GifCreator
{
     ...
     */
	public function create($frames = array(), $durations = array(), $loop = 0, $count, $font, $top, $left, $before, $after, $color, $text_size, $text_rotation)
    {
		...
		for ($i = 0; $i < count($frames); $i++) {
			...
			if (is_resource($frames[$i])) { // Resource var
                ...
                $resourceImg = $frames[$i];				$text = "Users online: " . $count;
				$red_background = imagecolorallocate($resourceImg, 255, 0, 0);
				$black = imagecolorallocate($resourceImg, 0, 0, 0);
				$width = imagesx($resourceImg);
				$height = imagesy($resourceImg);
				$text = $before . " " . $count . " " . $after; // store the text we're going to write in $text
				$leftTextPos = $left;
				$yellow = imagecolorallocate($resourceImg, 255, 255, 0);
				$white = imagecolorallocate($resourceImg, 255, 255, 255);
				$black = imagecolorallocate($resourceImg, 0, 0, 0);
				$colorha = imagecolorallocate($resourceImg, $color[0], $color[1], $color[2]);
				imagettftext($resourceImg, $text_size, $text_rotation, $left, $top, $colorha, $font, $text);
                ob_start();
                imagegif($frames[$i]);
